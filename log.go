...
func (this *LocalLogger) writeToLoggers(when time.Time, msg *loginfo, level int) {
    for _, l := range this.outputs {
        ...
        msgStr := when.Format(this.timeFormat) + " [" + msg.Level + "] " + pt + msg.Content
...
func (this *LocalLogger) writeMsg(logLevel int, msg string, v ...interface{}) error {
    ...
    if this.ShowCaller {
        ...
        if this.prefixPath != "" && this.useSimplePath {
            strim = this.prefixPath
            file = stringPathRemove(file, strim)
        }
...
func UseSimplePath(show bool) {
    defaultLogger.useSimplePath = show
    if defaultLogger.useSimplePath {
        defaultLogger.prefixPath, _ = os.Getwd()
    }
}
...
//替换项目路径
func stringPathRemove(s string, cut string) string {
